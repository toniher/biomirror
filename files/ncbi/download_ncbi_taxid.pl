...
my $ftpdir = "/pub/taxonomy";
print STDERR $ftpdir, "\n";


# other variables
my $base = ""; #Change depending on this is stored
my $data_dir = $base."/db/.mirror/ncbi/$currelease/taxonomy"; ####### CHANGE THIS TO THE DIRECTORY YOU WANT TO STORE YOUR FILES IN #######
my $stampfile = $base."/db/.mirror/ncbi/TAXONOMY";
my $data_link = $base."/db/.mirror/ncbi/current_taxonomy";

my $final_dir = $base."/db/ncbi/$currelease/taxonomy/db";
my $fstampfile = $base."/db/ncbi/$currelease/taxonomy/TAXONOMY";

my $emailbin = "~/bin/sendMsg.sh";

# Email Messages
my $subjsend = "Starting mirroring NCBI Taxonomy";
my $messagesend = "Please, be patient";

system ("$emailbin '$subjsend' '$messagesend'");

# List of included DB
my @listinclude = ('Ccode_dump.txt', 'Cowner_dump.txt', 'Icode_dump.txt', 'coll_dump.txt', 'gi_taxid.readme', 'gi_taxid_nucl.dmp.gz', 'gi_taxid_nucl_diff.dmp.gz', 'gi_taxid_prot.dmp.gz', 'gi_taxid_prot_diff.dmp.gz', 'taxcat.tar.gz', 'taxcat_readme.txt', 'taxdump.tar.gz', 'taxdump_readme.txt');

my @listmd5 = ('taxcat.tar.gz', 'taxdump.tar.gz');
...
if ($pextr > 0) {
	...
	my @filestar = grep {!/^\./ && -f "$data_dir/$_" && /tar\.gz$/} readdir($dh); #NOT EXTRACT MDSUMS
	@filestar = sort {$a cmp $b} @filestar;
	print STDERR @filestar, "\n";
	closedir $dh;
	
	opendir($dh, "$data_dir") or die "Can't opendir $data_dir: $!";
	my @filesgz = grep {!/^\./ && -f "$data_dir/$_" && /\.gz$/ && !/tar\.gz$/} readdir($dh); #NOT EXTRACT MDSUMS
	@filesgz = sort {$a cmp $b} @filesgz;
	print STDERR @filesgz, "\n";
	closedir $dh;

	opendir($dh, "$data_dir") or die "Can't opendir $data_dir: $!";
	my @filesot = grep {!/^\./ && -f "$data_dir/$_" && !/\.gz$/ && !/\.md5$/} readdir($dh); #NOT EXTRACT MDSUMS
	@filesot = sort {$a cmp $b} @filesot;
	print STDERR @filesot, "\n";
	closedir $dh;

	processtar(\@filestar, $data_dir, $final_dir);
	processgz(\@filesgz, $data_dir, $final_dir);
...
sub processtar {
	...
	foreach my $file (@{$files}) {
		...
		system("cp $orifile $endfile; tar zxf $endfile; rm $endfile") == 0 or die "tar gz failed: $?";
...
sub processgz {
	...
	foreach my $file (@{$files}) {
		...
		my $orifile = $origin."/".$file;
		system("zcat $orifile >> $endfile") == 0 or die "zcat failed: $?";
...
# Email Messages
my $subjsend2 = "Finished mirroring NCBI taxonomy";
